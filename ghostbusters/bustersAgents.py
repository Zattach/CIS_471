...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghost in livingGhostPositionDistributions:
            ...
            if(ghostDist < closeGhostDistance):
                closeGhostDistance = ghostDist
                closeGhost = ghostPos

        finalAction = None
        finalActionDistance = float("inf")
        for action in legal:
            nextPos = Actions.getSuccessor(pacmanPosition, action)
            tempDist = self.distancer.getDistance(nextPos, closeGhost)
            if(tempDist < finalActionDistance):
                finalActionDistance = tempDist
                finalAction = action
        return finalAction
